...
const updateCountry = asyncWrapper(async(req,res)=>{
    ...
    const existingCountry = await schema.findOne({countryName})
    if(existingCountry && existingCountry._id.toString() != id){
        ...
        if(req.file && req.file.fieldname === 'picture'){
            if(existingCountry.picture){
                fs.unlink(existingCountry.picture,(err)=>{
                    if(err){
                        return badGateway(res,`Error in server`,err)
                    }
                })
                updatedObject.picture = picture.path || existingCountry.picture;
            }
        }
